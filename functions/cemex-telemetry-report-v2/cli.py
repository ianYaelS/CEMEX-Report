...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="CEMEX telemetry V2: 30-second grid CSV (same logic as the Function)."
    )
    vehicle = parser.add_mutually_exclusive_group(required=True)
    vehicle.add_argument("--vehicle-id", dest="vehicle_id")
    vehicle.add_argument("--vehicle-name", dest="vehicle_name")
    ...
    parser.add_argument(
        ...
        choices=("30s", "minute", "native", "second"),
    )
    parser.add_argument("--csv-dialect", dest="csv_dialect", default=DEFAULT_CSV_DIALECT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = generate_report(params, token, storage=LocalStorage())
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_bytes(result.csv_bytes)
    print(f"wrote {output} ({result.row_count} rows) {result.validation_status}", flush=True)
    return 0
