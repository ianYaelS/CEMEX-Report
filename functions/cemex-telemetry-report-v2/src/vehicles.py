...
def _license_plate(data: dict[str, Any]) -> str:
    for key in ("licensePlate", "license_plate", "licensePlateNumber"):
        value = data.get(key)
        if value is None:
            continue
...
def _vehicle_from_item(item: dict[str, Any], fallback_id: str = "") -> VehicleRef | None:
    vehicle_id = str(item.get("id") or fallback_id).strip()
    if not vehicle_id:
        return None
    ...
    return VehicleRef(
        ...
        license_plate=_license_plate(item),
    )
...
def _vehicle_from_payload(payload: dict[str, Any], fallback_id: str) -> VehicleRef:
    ...
    vehicle = _vehicle_from_item(data, fallback_id)
    if vehicle is None:
...
def find_vehicle_by_name(client: SamsaraClient, vehicle_name: str) -> VehicleRef:
    ...
    for item in vehicles:
        ...
        if name == wanted:
            exact = VehicleRef(
                id=vehicle_id, name=name, license_plate=_license_plate(item)
            )
            break
...
def list_vehicles(client: SamsaraClient) -> list[VehicleRef]:
    """GET /fleet/vehicles (paginated): name, license plate, and id."""
    items: list[dict[str, Any]] = client.get_paginated(
        "/fleet/vehicles",
        {},
        _merge_vehicle_page,
    )
    ...
    seen: set[str] = set()
    for item in items:
        vehicle = _vehicle_from_item(item)
        if vehicle is None or vehicle.id in seen:
            continue
        seen.add(vehicle.id)
        vehicles.append(vehicle)
    vehicles.sort(key=lambda item: (item.name.lower(), item.license_plate.lower(), item.id))
    _log({"event": "vehicle_list", "ok": True, "count": len(vehicles)})
    return vehicles


def vehicle_label(vehicle: VehicleRef) -> str:
    plate = vehicle.license_plate or "sin placa"
    return f"{vehicle.name} · {plate} · {vehicle.id}"
