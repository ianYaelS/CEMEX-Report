...
from align import align_history, align_minute_window
from client import SamsaraClient
from constants import HISTORY_LOOKBACK_HOURS
from csv_writer import render_csv
from errors import ReportError
from minute_audit import INVALID, VALID, MinuteAudit, audit_minute_grid
...
def _build_day_report(
    ...
    rows, collapsed, out_of_range, carried = align_minute_window(
        history, params.timezone, day_start, day_end
    )
...
def _range_audit_payload(
    ...
    *,
    ...
    return {
        ...
        "rowCount": row_count,
        "expectedMinutes": len(daily_audits) * 1440,
...
def generate_report(
    ...
    *,
    ...
    try:
        ...
        cemex_minute = (not tracking) and params.granularity == "minute"
        day_windows = (
        ...
        if cemex_minute:
            ...
            expected_total = len(daily_audits) * 1440
            if any(count != 1440 for count in daily_row_counts.values()):
                raise ReportError(
                    f"each daily grid must have 1440 rows; got {daily_row_counts}"
                )
            if len(rows) != expected_total:
                raise ReportError(
                    f"rowCount must be {expected_total} "
                    f"({len(daily_audits)} days × 1440); got {len(rows)}"
                )
        else:
            rows = align_history(
                history,
        ...
        if should_store and backend is not None:
            ...
            if cemex_minute and daily_audits:
                first_audit_key = (
